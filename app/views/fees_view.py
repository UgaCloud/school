...
def _save_payment_form(form, *, bill, user):
    """Persist a validated payment consistently for page and AJAX workflows."""
    ...
    if not getattr(payment, "reference_no", None) or not str(payment.reference_no).strip():
...
def manage_bill_items_view(request):
    ...
    context = {
        ...
        "can_manage_student_bill": _can_manage_student_bill(request),
    }
...
def edit_bill_item_view(request,id):
    if not _can_manage_student_bill(request):
        messages.error(request, "Only Admin, Bursar, or Finance can edit bill items.")
        return HttpResponseRedirect(reverse(manage_bill_items_view))
...
def delete_bill_item_view(request, id):
    if not _can_manage_student_bill(request):
        messages.error(request, "Only Admin, Bursar, or Finance can delete bill items.")
        return HttpResponseRedirect(reverse(manage_bill_items_view))
    if request.method != "POST":
...
def manage_student_bills_view(request):
    ...
    academic_year_id = (
        ...
        or (str(current_academic_year.id) if current_academic_year else None)
    )
    selected_year = AcademicYear.objects.filter(pk=academic_year_id).first() if academic_year_id else None
    if not selected_year:
        selected_year = current_academic_year
        academic_year_id = str(selected_year.id) if selected_year else None

    term_id = request.GET.get('term') or request.session.get("academic_context_term_id")
    selected_term = (
        Term.objects.filter(pk=term_id, academic_year=selected_year).first()
        if term_id and selected_year else None
    )
    ...
    term_id = str(selected_term.id) if selected_term else None
    ...
    if academic_year_id:
        request.session["academic_context_year_id"] = str(academic_year_id)
    if term_id:
        request.session["academic_context_term_id"] = str(term_id)

    # Calculate summary statistics before pagination
    all_bills = student_bills  # Keep reference for statistics
    bill_rows = list(all_bills)
    for bill in bill_rows:
        bill.display_status = get_bill_status(bill)
    ...
    unpaid_bills = sum(bill.display_status.label in {"Unpaid", "Partial"} for bill in bill_rows)
    overdue_bills = sum(bill.display_status.label == "Overdue" for bill in bill_rows)

    # Pagination
    page = request.GET.get('page', 1)
    paginator = Paginator(bill_rows, 25)  # 25 bills per page
    ...
    context = {
        ...
        "selected_period_label": (
            f"{selected_year.academic_year} • Term {selected_term.term}"
            if selected_year and selected_term else "No academic period"
        ),
        "can_manage_student_bill": _can_manage_student_bill(request),
    }
...
def manage_student_bill_details_view(request, id):
    ...
    # Historical bill pages must show the period belonging to the bill, not
    # the student's current/admission period.
    context["academic_year"] = bill.academic_class.academic_year
    context["term"] = bill.academic_class.term
...
def add_student_payment_view(request, id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            _save_payment_form(form, bill=bill, user=request.user)
...
def carry_forward_balances_view(request):
    ...
    selected_student_id = request.POST.get("student") or request.GET.get("student") or ""
    ...
    if request.method == "POST" and request.POST.get("action") == "post":
        ...
        if not source_term or not target_term or source_term.id == target_term.id:
            ...
            return HttpResponseRedirect(
                f"{reverse(carry_forward_balances_view)}?source_term={source_term.id}&target_term={target_term.id}&class={selected_class_id}&active_only={'1' if active_students_only else '0'}&student={selected_student_id}"
            )
    ...
    context = {
        ...
        "selected_student": int(selected_student_id) if str(selected_student_id).isdigit() else "",
        "active_students_only": active_students_only,
...
def student_fees_status_view(request):
    ...
    selected_year_id = request.GET.get("year") or request.session.get("academic_context_year_id")
    ...
    term_obj = (
        Term.objects.filter(id=selected_term_id, academic_year=selected_year).first()
        if selected_term_id else None
    )
    if not term_obj:
        term_obj = Term.objects.filter(is_current=True, academic_year=selected_year).first()
    selected_term_id = str(term_obj.id) if term_obj else ""
    if selected_year:
        request.session["academic_context_year_id"] = str(selected_year.id)
    if term_obj:
        request.session["academic_context_term_id"] = str(term_obj.id)
    ...
    for bill in bills_qs.select_related("student", "academic_class", "academic_class__Class"):
        ...
        display_status = get_bill_status(bill, today=now)
        payment_status = display_status.label
        balance = display_status.balance
        balance_label = display_status.balance_label
        ...
        student_fees_data.append({
            ...
            "status_css_class": display_status.css_class,
    ...
    context = {
        ...
        "viewing_historical_period": bool(
            selected_year and term_obj
            and not (selected_year.is_current and term_obj.is_current)
        ),
        "selected_period_label": (
            f"{selected_year.academic_year} • Term {term_obj.term}"
            if selected_year and term_obj else "No academic period"
        ),
        "can_manage_student_bill": _can_manage_student_bill(request),
    }
...
def payment_ledger_view(request):
    ...
    if export_format in {"excel", "xlsx"}:
        workbook_bytes = build_ledger_workbook(
            ledger_payload["rows"],
            filters=active_filter_items,
            generated_by=getattr(request.user, "get_username", lambda: "System")(),
...
def student_fees_history_view(request, student_id):
    ...
    # Keep the complete account separate from the optional display filter so
    # the lifetime cards remain lifetime totals when a user selects one year.
    all_bills = list(
        StudentBill.objects
        ...
        .order_by('-academic_class__academic_year__academic_year',
                  '-academic_class__term__term')
    )

    lifetime_billed = sum((bill.total_amount for bill in all_bills), 0)
    lifetime_paid = sum((bill.amount_paid for bill in all_bills), 0)
    lifetime_outstanding = sum((bill.balance for bill in all_bills), 0)

    bills_qs = all_bills
    if selected_year_id:
        bills_qs = [
            bill for bill in all_bills
            if str(bill.academic_class.academic_year_id) == str(selected_year_id)
        ]
    ...
    for bill in bills_qs:
        ...
        years_data[year_key]["year_outstanding"] += bill.balance
...
def upload_bill_document(request, id):
    ...
    if not _can_manage_student_bill(request):
        messages.error(request, "Only Admin, Bursar, or Finance can upload bill documents.")
        return HttpResponseRedirect(reverse(manage_student_bill_details_view, args=[bill.id]))
...
def delete_bill_document(request, id):
    ...
    if not _can_manage_student_bill(request):
        messages.error(request, "Only Admin, Bursar, or Finance can delete bill documents.")
