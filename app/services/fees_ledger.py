...
def get_student_reg_no(student):
    # Backwards-compatible alias used by existing templates/context keys.
...
def _apply_common_filters(
    ...
    *,
    ...
    # Individual classroom filtering is required by the client for KG1-KG3,
    # GD1-GD7 and Tahfidh. This is query-only and does not touch stored data.
    code = str(classroom_code or "").strip().upper()
    if code:
        if "TAHF" in code:
        ...
        return queryset.filter(
            Q(bill__academic_class__Class__code__iexact=code)
            | Q(bill__academic_class__Class__name__iexact=code)
        )
...
def _format_filter_summary(filters):
    if not filters:
...
def _build_fallback_workbook_bytes(rows, *, filters=None, generated_by="", generated_at=""):
    headers = _ledger_export_headers()
    metadata_rows = [
        ["Student Payment Transaction Ledger"],
        ["Generated By", generated_by or "System"],
        ["Generated On", generated_at or ""],
        ["Filters", _format_filter_summary(filters)],
        [],
        headers,
    ]

    row_xml = []
    for row_index, header_row in enumerate(metadata_rows, start=1):
        cells = [
    ...
    first_data_row = len(metadata_rows) + 1
    for row_index, row in enumerate(rows, start=first_data_row):
        values = _ledger_export_values(row)
        cells = []
...
def build_ledger_workbook(rows, *, filters=None, generated_by="", generated_at=None):
    generated_at_text = ""
    if generated_at:
        generated_at_text = generated_at.strftime("%Y-%m-%d %H:%M") if getattr(generated_at, "strftime", None) else str(generated_at)

    if Workbook is None:
        return _build_fallback_workbook_bytes(
            rows,
            filters=filters,
    ...
    metadata_rows = [
        ["Student Payment Transaction Ledger"],
        ["Generated By", generated_by or "System"],
        ["Generated On", generated_at_text],
        ["Filters", _format_filter_summary(filters)],
        [],
        headers,
    ]
    ...
    for row_index in (2, 3, 4):
        sheet.cell(row=row_index, column=1).font = metadata_font

    header_row_index = len(metadata_rows)
    header_fill = PatternFill("solid", fgColor="1F4E78")
    header_font = Font(color="FFFFFF", bold=True)

    for column_index, header in enumerate(headers, start=1):
        cell = sheet.cell(row=header_row_index, column=column_index)
    ...
    for column_index in (10, 11, 12):
        for cell in sheet.iter_cols(min_col=column_index, max_col=column_index, min_row=header_row_index + 1):
            for item in cell:
                item.number_format = '#,##0'

    widths = [14, 18, 22, 16, 28, 14, 18, 16, 18, 18, 18, 20, 36]
    for column_index, width in enumerate(widths, start=1):
        sheet.column_dimensions[_column_letter(column_index)].width = width

    sheet.freeze_panes = f"A{header_row_index + 1}"
    sheet.auto_filter.ref = f"A{header_row_index}:{_column_letter(len(headers))}{max(header_row_index, sheet.max_row)}"
